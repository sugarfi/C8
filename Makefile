...
LD=ld
CFLAGS=-fno-stack-protector -mgeneral-regs-only -mno-red-zone -ffreestanding -fno-pie -m32 -c -I src/kernel -I src/kernel/lib -nostdinc -nostdlib -std=c99 -Os
LDFLAGS=-T linker.ld -melf_i386
CSOURCES=$(shell find src/kernel -name '*.c')
COBJECTS=$(CSOURCES:.c=.o)

bin/boot.bin: src/boot/boot.asm src/boot/pmode.asm
	@mkdir bin -p
...
all: bin/boot.bin bin/kernel.bin
	...
	@echo "Write test.txt to os.img"
	@dd if=test.txt of=os.img conv=notrunc seek=16
